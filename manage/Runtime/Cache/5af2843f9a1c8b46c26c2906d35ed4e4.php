    ...
    <div class="left-side sticky-left-side">
        ...
        <div class="left-side-inner">
            ...
            <ul class="nav nav-pills nav-stacked custom-nav">
                ...
                <li class="menu-list <?php if((MODULE_NAME) == "Hero"): ?>nav-active<?php endif; ?> ">
                    <a href="javascript:;"><i class="fa fa-file-text"></i> <span>召唤英雄管理</span></a>
                    <ul class="sub-menu-list">
                        <li class=" <?php if(($_GET["gid"]) == "1"): ?>active<?php endif; ?> "><a href="<?php echo U('Summonhero/optionlist',array(gid=>1));?>">LOL赔率</a></li>
                        <li class=" <?php if(($_GET["gid"]) == "2"): ?>active<?php endif; ?> "><a href="<?php echo U('Summonhero/optionlist',array(gid=>2));?>">Dota2赔率</a></li>
                    </ul>
                </li>
        ...
        <div class="wrapper">
			...
			<div class="row">
                ...
                <header class="panel-heading">
					<?php if(($_GET["gid"]) == "1"): ?>LOL赔率<?php endif; ?>
					<?php if(($_GET["gid"]) == "2"): ?>Dota2赔率<?php endif; ?>
                </header>
                ...
                <tbody>
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$l): $mod = ($i % 2 );++$i;?><tr class="">
							<td><?php echo ($i); ?></td>
							<td><?php echo ($l["id"]); ?></td>
							<td><?php echo ($l["shuxing_name"]); ?></td>
							<td><?php echo ($l["peilv"]); ?></td>
							...
							<td>
								...
								|
								<?php if($l['state']==1){ ?>
									<a href="javascript:;" onclick="del_admin(<?php echo ($l["id"]); ?>,'roulette_peilv',this,'id')">删除</a>
								<?php }else{ ?>
									<a href="javascript:;" onclick="hui_admin(<?php echo ($l["id"]); ?>,'roulette_peilv',this,'id')">恢复</a>
									|
									<a href="javascript:;" class="cddel_admin_btn" onclick="cddel_admin(<?php echo ($l["id"]); ?>,'roulette_peilv',this,'id')">彻底删除</a>
								<?php } ?>
							</td>
						</tr>
						<!--编辑框 start-->
							...
							<div class="modal-dialog">
								<div class="modal-content">
									...
									<div class="modal-body ">
										<form action="<?php echo U('Hero/peilv_edit');?>" method="post">
											<input type="hidden" name="id" value="<?php echo ($l["id"]); ?>">
											<input type="hidden" name="gid" value="<?php echo ($_GET["gid"]); ?>">
											<div class="form-group">
												<label>选择属性</label>													
											</div>
											<?php if(is_array($shuxingtypelist)): $i = 0; $__LIST__ = $shuxingtypelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$s): $mod = ($i % 2 );++$i;?><div class="form-group">
													<label>
														<?php echo ($s["name"]); ?>
													</label>
													<label>
														<?php if(is_array($s["xinglist"])): $i = 0; $__LIST__ = $s["xinglist"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$x): $mod = ($i % 2 );++$i;?><input name="shuxing[]" type="checkbox" value="<?php echo ($x["roulette_propertyId"]); ?>" <?php if(in_array($x['roulette_propertyId'],$l['shuxing_arr'])){ ?> checked="checked"<?php } ?>><?php echo ($x["name"]); ?>&nbsp;<?php endforeach; endif; else: echo "" ;endif; ?>
													</label>
												</div><?php endforeach; endif; else: echo "" ;endif; ?>
											<div class="form-group">
												<label>赔率</label>
												<input name="peilv" value="<?php echo ($l["peilv"]); ?>" class="form-control">
											</div>
					...
					<div class="modal-body">
						<form action="<?php echo U('Hero/peilv_edit');?>" method="post">
							...
							<?php if(is_array($shuxingtypelist)): $i = 0; $__LIST__ = $shuxingtypelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$s): $mod = ($i % 2 );++$i;?><div class="form-group">
									...
									<label>
										<?php if(is_array($s["xinglist"])): $i = 0; $__LIST__ = $s["xinglist"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$x): $mod = ($i % 2 );++$i;?><input name="shuxing[]" type="checkbox" value="<?php echo ($x["roulette_propertyId"]); ?>"><?php echo ($x["name"]); ?>&nbsp;<?php endforeach; endif; else: echo "" ;endif; ?>
									</label>
								</div><?php endforeach; endif; else: echo "" ;endif; ?>
							<div class="form-group">
								<label>赔率</label>
								<input name="peilv" value="" class="form-control">
							</div>
							<button type="submit" class="btn btn-primary">保存</button>
						</form>
					</div>
