             ...
             <div class="row">
                ...
                <div class="clearfix">
                    <div class="btn-group">
						...
						<a href="<?php echo U('Tupian/tupian_changetime');?>" target="_blank" >
							变时间
                        </a>
                ...
                <tbody>
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$l): $mod = ($i % 2 );++$i;?><tr class="">
							<td><?php echo ($i); ?></td>
							<td><?php echo ($l["teamId"]); ?></td>
							<td><?php echo ($l["name"]); ?></td>
							<td><img src="<?php echo ($l["avatar"]); ?>" height="40" /></td>
							<td><?php echo ($l["gname"]); ?></td>
							<td>
								<a class="tan_edit" data-toggle="modal" href="#bianjibox<?php echo ($l["teamId"]); ?>">编辑</a>
								|
								<?php if($l['state']==1){ ?>
									<a href="javascript:;" onclick="del_admin(<?php echo ($l["teamId"]); ?>,'teams',this,'teamId')">删除</a>
								<?php }else{ ?>
									<a href="javascript:;" onclick="hui_admin(<?php echo ($l["teamId"]); ?>,'teams',this,'teamId')">恢复</a>
									|
									<a href="javascript:;" class="cddel_admin_btn" onclick="cddel_admin(<?php echo ($l["teamId"]); ?>,'teams',this,'teamId')">彻底删除</a>
								<?php } ?>
							</td>
							<!--编辑框 start-->
							<div aria-hidden="true" aria-labelledby="myModalLabel" role="dialog" tabindex="-1" id="bianjibox<?php echo ($l["teamId"]); ?>" class="modal fade">
								<div class="modal-dialog">
									<div class="modal-content">
										...
										<div class="modal-body ">
											<form enctype="multipart/form-data" action="<?php echo U('Tupian/tupian_edit');?>" method="post">
												<input type="hidden" name="id" value="<?php echo ($l["teamId"]); ?>">
												<div class="form-group">
													<label>名称：</label>
													<input id="name" name="name" value="<?php echo ($l["name"]); ?>" class="form-control">
												</div>
												<div class="form-group">
													<label for="exampleInputFile">头像：</label>
													<p>
														<img src="<?php echo ($l["avatar"]); ?>" width="40" />
													</p>
													<input type="file" id="imgurl" name="img">
													<p class="help-block">大小不超过500KB</p>
												</div>
												<div class="form-group">
													<label>所属分类</label>
													<select name="gid" class="form-control">
														<?php if(is_array($fenleis)): $ii = 0; $__LIST__ = $fenleis;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$g): $mod = ($ii % 2 );++$ii;?><option value="<?php echo ($g["game_typeId"]); ?>"  <?php if($g['game_typeId']==$l['gid']){ ?>selected="selected"<?php } ?>><?php echo ($g["cnName"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
													</select>
												</div>
			...
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-body">
						...
						<form enctype="multipart/form-data" role="form" action="<?php echo U('Tupian/tupian_edit');?>" method="post" >
							...
							<div class="form-group">
								<label>所属分类</label>
								<select name="gid" class="form-control">
									<?php if(is_array($fenleis)): $i = 0; $__LIST__ = $fenleis;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$g): $mod = ($i % 2 );++$i;?><option value="<?php echo ($g["game_typeId"]); ?>"  <?php if($g['game_typeId']==$l['gid']){ ?>selected="selected"<?php } ?>><?php echo ($g["cnName"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
								</select>
							</div>
							<button type="submit" class="btn btn-primary">保存</button>
						</form>
